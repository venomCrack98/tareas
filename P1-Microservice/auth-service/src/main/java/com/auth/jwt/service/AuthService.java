...
import com.auth.jwt.repository.AuthUserRespository;
...
public class AuthService {
    ...
    public AuthUser save (NewUserDto dto){
        Optional<AuthUser> user = authUserRespository.findByUserName(dto.getUserName());
        if(user.isPresent()){
            return null;
        }
        ...
        AuthUser authUser = AuthUser.builder()
                ...
                .role(dto.getRole())
                .build();
        return authUserRespository.save(authUser);
    }
    ...
    public TokenDto validate(String token, RequestDto requestDto){
        ...
        if(!authUserRespository.findByUserName(userName).isPresent()){
            return null;
        }
